...
fn main() {
    // ── 0. Check status bar configuration ───────────────────────────────
    hooks::init_status_config();
    ...
    if h_dll.is_null() {
        ...
        eprintln!(
            "[srcds_console] Failed to load {}: error 0x{:08x}",
            dll_rel, err
        );
    ...
    // ── 5b. Set up terminal scroll region for status bar ────────────────
    unsafe { hooks::setup_scroll_region(); }
    ...
    // ── 8. Cleanup terminal and exit ────────────────────────────────────
    unsafe { hooks::cleanup_scroll_region(); }
    std::process::exit(exit_code);
}

/// Get game root directory from this executable's location.
...
fn prepend_path(dir: &std::path::Path) {
    ...
    unsafe {
        SetEnvironmentVariableA(key.as_ptr() as *const u8, val.as_ptr() as *const u8);
    }
}

/// Convert a PathBuf to a CString for Windows API calls.
...
/// Print a human-readable Windows error message.
